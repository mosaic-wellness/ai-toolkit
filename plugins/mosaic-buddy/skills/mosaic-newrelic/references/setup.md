...
`/mosaic-buddy tools-init newrelic`.

The token lives in `~/.config/mosaic-buddy/tokens.env` as
`NEW_RELIC_API_KEY`.
...
4. Fill the form:
   ...
   - **Notes**: optional — e.g. "Used by mosaic-buddy Claude Code plugin".
...
- Writes `NEW_RELIC_API_KEY=` to `~/.config/mosaic-buddy/tokens.env`.
...
```
/mosaic-buddy tools-init validate
```
...
1. `/mosaic-buddy tools-init rotate newrelic` and paste a fresh key.
...
2. `/mosaic-buddy tools-init remove newrelic` to clean up
   `tokens.env`.
