# Implementation Research: kai Plugin Redesign
...
**Action:** Move from `plugins/code-review/references/` to `plugins/kai/references/`

**Additions needed for kai context:**
...
description: >
  ...
  "/kai brainstorm" (help me plan), "/kai 10x" (weekly coaching report).
user-invocable: true
...
Check for prior kai artifacts (docs/ folder, report files). If none → first-run greeting.
...
of data — best run once a week.
  ...
  Scope: [project name] (use '/kai 10x all' for everything)
...
Open: `open kai-coaching-report.html` (macOS).
...
*This research document contains enough detail to write every file in the kai plugin redesign. Each section provides exact detection patterns, output formats, tone rules, and behavioral specifications.*
