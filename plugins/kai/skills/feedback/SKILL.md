...
description: >
  ...
  with the shared HMAC key. Honours KAI_TELEMETRY_URL=off.
---

# Feedback

The user invoked `/kai feedback`. Drive a short three-step conversation, then submit.
...
{
  "questions": [{
    "question": "How is kai going for you?",
    "header": "Rating",
    "multiSelect": false,
    "options": [
...
- **On failure (non-zero):** show the script's stderr verbatim so the user can see what went wrong (most common: server down, or `KAI_TELEMETRY_URL=off`). Offer to retry once.
...
5. **One submission per invocation.** If the user wants to add more, they can re-run `/kai feedback`.
