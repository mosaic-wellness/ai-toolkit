...
name: handoff
description: >
  One-command session save and resume. `/kai handoff <sessionName>`
...
| `/kai handoff` (no args)                                                               | Ask the user |
...
Write a durable summary of the current Claude Code session to **`<projectRoot>/work-log/<sessionName>/session-N.md`** so the work can be resumed cleanly via `/kai handoff <sessionName>` in a fresh session.
...
- How to resume later: `/kai handoff <sessionName>` in a fresh session (no `new` keyword)
...
Pick up a prior session that was checkpointed via `/kai handoff`. Read the structured handoff file(s) from `<projectRoot>/work-log/<sessionName>/`, verify the repo state is still consistent, and report the next concrete step.
...
> Run `/rename <sessionName>` to label this resumed session so the next `/kai handoff` call keeps everything under the same work-log folder.
...
If the user later wants to checkpoint *their own* progress in this session, the next `/kai handoff <sessionName>` call will still resume (the latest file belongs to the original author, not this session). In that case, suggest a slightly different name — e.g. `<sessionName>-pt2` — to start a fresh chain.
